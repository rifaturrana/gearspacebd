...
{
    // Get the deepest category (assuming the last one is the deepest)
    $deepestCategory = $this->categories()->orderByDesc('id')->first();

    if (!$deepestCategory) {
        return self::whereRaw('0 = 1'); // Return an empty query
    }

    // Get all descendant category IDs (including itself)
    $categoryIds = [$deepestCategory->id];

    // If you have a method to get children recursively, use it here.
    // For now, let's assume only the current category.
    // If you have a method like $deepestCategory->descendants()->pluck('id'), merge those IDs in.

    // Get all products in these categories, excluding this product
